...
function element<T extends Element>(selector: string): T {
  ...
  if (found === null) throw new Error(`Missing required element: ${selector}`);
  return found;
}

const token = element<HTMLMetaElement>('meta[name="seaborg-token"]').content;
const boardElement = element<HTMLElement>("#board");
...
function squareButton(square: string): HTMLButtonElement | null {
  return boardElement.querySelector<HTMLButtonElement>(`.square[data-square="${square}"]`);
}
...
function renderBoard(
  ...
  const checkedKing = state.inCheck
    ? [...board.values()].find(
        (piece) => piece.color === state.sideToMove && piece.kind === "king",
      )?.square ?? null
    : null;
  ...
  for (const [index, square] of squares.entries()) {
    ...
    if (button.tabIndex === 0) focusedSquare = square;

    if (square === selectedSquare) button.classList.add("selected");
    if (square === lastFrom || square === lastTo) button.classList.add("last-move");
    if (square === checkedKing) button.classList.add("in-check");
    const target = selectedMoves.find((move) => move.to === square);
    if (target !== undefined) button.classList.add(target.capture ? "legal-capture" : "legal-target");
...
function activateSquare(square: string): void {
  ...
  if (state === null || !canInteract(state)) return;
  const board = parseFen(state.fen);
...
async function choosePromotion(candidates: readonly MoveOption[]): Promise<MoveOption | null> {
  const allowed = new Map(candidates.map((move) => [move.promotion, move]));
  for (const button of promotionDialog.querySelectorAll<HTMLButtonElement>("[data-promotion]")) {
    const promotion = button.dataset.promotion ?? "";
    button.disabled = !allowed.has(promotion as MoveOption["promotion"]);
  }
  promotionDialog.returnValue = "";
  promotionDialog.showModal();
  await new Promise<void>((resolve) => promotionDialog.addEventListener("close", () => resolve(), { once: true }));
  return allowed.get(promotionDialog.returnValue as MoveOption["promotion"]) ?? null;
}

async function playCandidates(candidates: readonly MoveOption[]): Promise<void> {
...
function animateSnapback(from: string, to: string, piece: Piece | undefined): void {
...
function closestSquare(target: EventTarget | null): HTMLButtonElement | null {
  return target instanceof Element ? target.closest<HTMLButtonElement>(".square") : null;
}

boardElement.addEventListener("pointerdown", (event) => {
  const state = snapshot;
  const square = closestSquare(event.target);
  if (state === null || square === null || !canInteract(state)) return;
...
boardElement.addEventListener("pointermove", (event) => {
  ...
  const state = snapshot;
  if (active === null || state === null || active.pointerId !== event.pointerId || !active.draggable) return;
...
boardElement.addEventListener("keydown", (event) => {
  ...
  if (state === null || current === undefined) return;
  ...
  if (movement !== undefined) {
    ...
    if (next !== null) {
      ...
      for (const button of boardElement.querySelectorAll<HTMLButtonElement>(".square")) button.tabIndex = -1;
      if (nextButton !== null) {
...
async function loadInitialState(): Promise<void> {
  try {
    ...
    if (response.ok) render((await response.json()) as Snapshot);
  } catch {
