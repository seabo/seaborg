...
export function shouldAdopt(
  current: { readonly revision: number } | null,
  incoming: { readonly revision: number },
): boolean {
  return current === null || incoming.revision >= current.revision;
}

/** What became of a command: answered, refused by the server, or never answered at all. */
export type CommandOutcomeKind = "ok" | "rejected" | "unreachable";
 ...
 */
export function quitEndsTheSession(outcome: CommandOutcomeKind): boolean {
...
export function formatCount(value: number): string {
  ...
  const units: ReadonlyArray<{ readonly scale: number; readonly suffix: string }> = [
  ...
  for (const { scale, suffix } of units) {
    ...
    if (Number(rendered) < 1_000) return `${rendered}${suffix}`;
  }
