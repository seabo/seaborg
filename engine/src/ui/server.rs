...
/// How long draining a rejected request may block before the connection is dropped regardless.
...
/// the remainder first allows an ordinary close. The cap and the short timeout keep a client that
/// keeps sending from occupying this thread.
...
/// 127.0.0.1 so the browser treats this server as that site's origin. `Origin` is validated
/// whenever present; browsers always send it on cross-origin requests, and its absence means the
/// caller is not a browser, which the token requirement covers separately.
